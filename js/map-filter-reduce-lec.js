...
$(document).ready(function() {
    ...
    /********************************* 3 *********************************/
        //  3. Use ES6
    let filter2011ES6 = mbpModels.filter(mbp => getYear(mbp.year) >= 2011);
    $('#filterEs6').html(createList(filter2011ES6.map(mbp => `${mbp.desc} <strong> (${mbp.year})</strong>`)));
    // *****************   ******     ******************//
    // **************  TODO: REDUCE() ******************//
    // *****************   ******     ******************//
    ...
    //  1. Use forEach()
    let totalValueFe = 0;
    mbpModels.forEach(function(mpb){
        totalValueFe+=mpb.price;
    });
    $('#reduceForEach').html(commaThousands(totalValueFe))
    /********************************* 2 *********************************/
    //  2. Use reduce()
  let e6reducee = mbpModels.reduce(function (total, mpb) {
return total + mpb.price
        },0);
    $('#reduce').html(commaThousands(e6reducee));

    /********************************* 3 *********************************/
    //  3. Use ES6

   let e6reduce = mbpModels.reduce((total, mbp) => total + mbp.price, 0);
    $('#reduceEs6').html(commaThousands(e6reduce));
